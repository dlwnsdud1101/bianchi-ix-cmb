...
def run_mc(N, seed=2026, theta_cmb=THETA_CMB_OBS, theta_cf4=THETA_CF4_OBS):
    ...
    Conditions (from paper §2.4):
        ...
        (iii) CMB  ↔ thermal gradient > 55°  ← auto-satisfied given (i)
        (iv)  CF4  ↔ thermal gradient > 55°  ← auto-satisfied given (ii)

    Because SGP ↔ thermal = 62°, conditions (iii)+(iv) are geometrically
    guaranteed whenever (i)+(ii) hold → effective DOF = 2.
    """
    ...
    # Verify geometric auto-satisfaction of (iii) and (iv)
    sgp_therm_angle = np.degrees(np.arccos(np.clip(abs(n_sgp @ n_thermal), 0, 1)))
    auto_iii = (sgp_therm_angle > THETA_THERM)   # True if guaranteed

    joint_2  = cond_i  & cond_ii
    p_2cond  = joint_2.mean()

    return p_2cond, sgp_therm_angle, auto_iii
...
if __name__ == "__main__":
    ...
    p_2, sgp_therm, auto = run_mc(N, seed)

    p_bonf = min(p_2 * BONFERRONI_N, 1.0)
    sig_2  = p_to_sigma(p_2)
    sig_b  = p_to_sigma(p_bonf)

    print(f"SGP ↔ thermal gradient     : {sgp_therm:.1f}°  "
          f"(auto-condition {'✓' if auto else '✗'} > {THETA_THERM}°)")
    print()
    print(f"{'Condition':<35} {'P (%)':>10} {'σ':>6}")
    print("─" * 54)
    print(f"{'2-cond (CMB+CF4 → SGP)':<35} {p_2*100:>10.5f} {sig_2:>6.2f}")
    print(f"{'Bonferroni-corrected (×6)':<35} {p_bonf*100:>10.5f} {sig_b:>6.2f}")
    print()
